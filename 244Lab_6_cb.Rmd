...
ca_dams$Condition <- fct_relevel(ca_dams$Condition, "Fair", "Satisfactory", "Unsatisfactory", "Poor") # Easy way to relevel (this will be important later)
...
ggplot(ca_eco) +
  ...
  geom_sf(data = ca_counties, # Must specify the data here
          fill = "NA",
          color = "gray30",
          size = 0.1) + # Size of line
  geom_point(data = ca_dams,
             ...
             alpha = 0.5) +
  theme_minimal() +
  coord_sf(datum = NA) # Remove coordinate lines
...
sn <- ca_eco %>% # polygons
  ...
  st_join(ca_dams) # Join the info from previous line with info in ca_dams
...
ggplot(eco_clip) +
  ...
  theme_minimal() +
  theme(legend.position = c(0.5, 0.015)) +
...
# Example with a different basemap (use leaflet::providers to find the basemap names)
tm_basemap("JusticeMap.plurality") +
  tm_shape(eco_clip) +
...
sb_faults <- fault_lines %>% 
  st_intersection(sb) # the other option is join - shown in example above
...
tm_basemap("CartoDB.DarkMatter") +
  tm_shape(sb) +
  tm_borders(col = "gray50", lwd = 2) + # lwd = line width
  tm_shape(sb_faults) +
...
sites_sf <- st_as_sf(ca_sites, coords = c("LONDD", "LATDD"), crs = 4326) # Order matters here! R needs Longitude and the Latitude
...
ca_tot$n[is.na(ca_tot$n)] <- 0 # we don't want it to show up as no data.. 
